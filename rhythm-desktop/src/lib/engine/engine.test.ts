...
test('勿扰时段（午休）：落在其中不触发，nextFireTime 顺延到结束后', () => {
  const r = noGateReminder('eye'); // workWindow 已关，20min 周期
  r.quietWindows = [{ enabled: true, start: '12:00', end: '13:00', days: [1, 2, 3, 4, 5] }];
  const s = createInitialState(r, 1_000_000);
  // 构造一个 12:30 的上下文（周一=1）
  const ctx = ctxAt(new Date(2024, 0, 1, 12, 30, 0).getTime()); // 2024-01-01 是周一
  const res = shouldFire(r, s, ctx, getLocal);
  assert.equal(res.fire, false);
  assert.equal(res.reason, 'quiet');
  // nextFireTime 应跳到 13:00 之后
  const nxt = nextFireTime(r, s, ctx, getLocal);
  const nextClock = getLocal(nxt).clock;
  assert.ok(nextClock >= '13:00', `应顺延到午休后，实际 ${nextClock}`);
});

test('严格模式：skip 被引擎拒绝', () => {
  const store = new MemStore();
  const engine = new RhythmEngine(store, getLocal, () => 1_000_000);
  const r = noGateReminder('eye');
  r.id = 's1';
  r.gates.strictMode = true;
  engine.add(r);
  const s = engine.getState('s1')!;
  const before = s.phaseEndsAt;
  engine.skip('s1');
  // 严格模式下 skip 应被忽略，phaseEndsAt 不变
  assert.equal(engine.getState('s1')!.phaseEndsAt, before);
});

test('严格模式：snooze 被引擎拒绝', () => {
  const store = new MemStore();
  const engine = new RhythmEngine(store, getLocal, () => 1_000_000);
  const r = noGateReminder('eye');
  r.id = 's2';
  r.gates.strictMode = true;
  engine.add(r);
  const s = engine.getState('s2')!;
  const before = s.phaseEndsAt;
  engine.snooze('s2', 5);
  // 严格模式下 snooze 应被忽略，phaseEndsAt 不变
  assert.equal(engine.getState('s2')!.phaseEndsAt, before);
});

test('非严格模式：skip/snooze 正常执行', () => {
  const store = new MemStore();
  const engine = new RhythmEngine(store, getLocal, () => 1_000_000);
  const r = noGateReminder('eye');
  r.id = 's3';
  r.gates.strictMode = false;
  engine.add(r);
  const s = engine.getState('s3')!;
  const before = s.phaseEndsAt;
  engine.skip('s3');
  // 非严格模式 skip 应生效（20min 的工作段后）
  assert.equal(engine.getState('s3')!.phaseEndsAt, 1_000_000 + 20 * 60000);
  // snooze 也应正常：延后 10min 从 now 算起（而非从已跳过的 phaseEndsAt 叠加）
  engine.snooze('s3', 10);
  assert.equal(engine.getState('s3')!.phaseEndsAt, 1_000_000 + 10 * 60000);
});
